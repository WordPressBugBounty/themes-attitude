			...
			<div class="social-profiles clearfix">
				...
				foreach( $social_links as $key => $value ) {
					if ( !empty( $options[ $value ] ) ) {
						$attitude_socialnetworks .=
							'<li class="'.strtolower($key).'"><a href="'.esc_url( $options[ $value ] ).'" title="'.sprintf( esc_attr__( '%1$s on %2$s', 'attitude' ), get_bloginfo( 'name' ), $social_links_name[$i] ).'" target="_blank" rel="noopener noreferrer">'.get_bloginfo( 'name' ).' '.$social_links_name[$i].'</a></li>';
					}
					$i++;
				}		
